...
hook 배경 사진은 캔버스(1080x1350) 전체가 아니라 위쪽 사진 영역
(1080xHOOK_PHOTO_AREA_HEIGHT)에만 ImageOps.fit()으로 맞춰 배치된다.
...
from card_config import CANVAS_WIDTH, HOOK_PHOTO_AREA_HEIGHT

MAX_MAGNIFICATION = 1.5  # 이 배율을 넘게 확대해야 사진 영역을 채울 수 있으면 저화질로 본다
...
def check_resolution(image, label):
    ...
    magnification = max(CANVAS_WIDTH / width, HOOK_PHOTO_AREA_HEIGHT / height)
