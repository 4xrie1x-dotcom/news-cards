...
# 의원 프로필 사진엔 공공누리 마크가 없어 자유이용 대상이 아님을 확인했다(2026-08-16).
# CLAUDE.md의 "저작권 프리 소스에서만 조달" 원칙에 어긋나 위키미디어를 1순위로 되돌린다.

# 이름 뒤에 이 직함이 붙어 있으면 "요약에 언급된 주요 인물"로 본다(직함 바로 앞
# 글자만 이름으로 보는 단순 규칙). 정당명은 미리 지운다. "시장"은 "서울시장"(직함)과
# "시장 친화적"(경제 용어) 둘 다로 쓰여 오인 버그가 있었다(2026-08-17) — 지역명이
# 바로 앞에 붙어야만("인천시장") 직함으로 본다.
PERSON_TITLE_MARKERS = ["의원", "대표", "장관", "대통령", "총리", "위원장", "지사", "원내대표"]
REGION_NAMES = [
    "서울", "부산", "대구", "인천", "광주", "대전", "울산", "세종",
    "경기", "강원", "충북", "충남", "전북", "전남", "경북", "경남", "제주",
]
CITY_MAYOR_MARKER = "(?:" + "|".join(REGION_NAMES) + ")시장"
TITLE_GROUP = "(?:" + "|".join(PERSON_TITLE_MARKERS) + f"|{CITY_MAYOR_MARKER})"
...
# "Korea national assembly"는 경복궁·광화문과 오매칭됐다(2026-08-17 확인) — 여의도·
# 실제 건물명을 넣어 구분한다.
CONTEXT_KEYWORD_MAP = [
...
def get_hook_background(summary_json):
    """hook 카드 배경 사진을 조달한다. 인물명이 있으면 위키백과 인포박스→위키미디어
    커먼즈→Pexels 순으로 시도하고, 모두 실패하면 (None, None)을 반환해 hook_card.py가
    텍스트 전용 fallback을 쓰게 한다. (사진 경로, 캡션용 출처 표시) 튜플을 반환한다."""
